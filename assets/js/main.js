...
$(".tableImg span img").click(function(){
...
function getFlag2() {
	flagX2 = event.touches[0].pageX;
	flagXM2 = flagX;
}

function setFlag2() {
	flagXM2 = event.touches[0].pageX;
}

function change2() {
	var range = flagXM2 - flagX2;
	//如果水平滑动距离小于30，就不切换
	if (Math.abs(range) < 30) {
		return false;
	}
	var direction = range < 0 ? 'next' : 'prev';
	$("#carousel-example-generic2").carousel(direction);
...
function getFlag3() {
	flagX3 = event.touches[0].pageX;
	flagXM3 = flagX;
}

function setFlag3() {
	flagXM3 = event.touches[0].pageX;
}

function change3() {
	var range = flagXM3 - flagX3;
	//如果水平滑动距离小于30，就不切换
	if (Math.abs(range) < 30) {
		return false;
	}
	var direction = range < 0 ? 'next' : 'prev';
	$("#carousel-example-generic3").carousel(direction);
